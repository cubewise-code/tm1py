...
class CellService(ObjectService):
    ...
    def get_value(
        ...
        **kwargs,
        ...
        for dimension_name, element_selection in zip(
            dimensions[:-1], element_selections[:-1]
        ):
            if "&&" not in element_selection:
                mdx_rows_list.append(
                    "{["
                    + dimension_name
                    + "].["
                    + dimension_name
    ...
    def relative_proportional_spread(
        ...
        **kwargs,
        ...
        payload = {
            ...
            "ReferenceCube@odata.bind": format_url(
                "Cubes('{}')", reference_cube if reference_cube else cube
            ),
        }
        ...
        return self._post_against_cellset(
            cellset_id=cellset_id, payload=payload, delete_cellset=True, **kwargs
        )

    def clear_spread(
        self, cube: str, unique_element_names: Iterable[str], **kwargs
    ) -> Response:
    ...
    def clear_with_mdx(self, cube: str, mdx: str, **kwargs):
        ...
        try:
            success, _, _ = process_service.execute_process_with_return(
                process, **kwargs
            )
            if not success:
                raise TM1pyException(
                    f"Failed to clear cube: '{cube}' with mdx: '{abbreviate_mdx(mdx, 100)}'"
                )
        finally:
    ...
    def _post_against_cellset(
        self, cellset_id: str, payload: Dict, **kwargs
    ) -> Response:
    ...
    def write_value(
        ...
        **kwargs,
        ...
        body_as_dict["Cells"][0]["Tuple@odata.bind"] = [
            format_url(
                "Dimensions('{}')/Hierarchies('{}')/Elements('{}')", dim, dim, elem
            )
    ...
    def write_values_through_cellset(
        self, mdx: str, values: List, **kwargs
    ) -> Response:
    ...
    def execute_view(
        ...
        **kwargs,
        ...
        cellset_id = self.create_cellset_from_view(
            cube_name=cube_name, view_name=view_name, private=private, **kwargs
        )
        return self.extract_cellset(
            cellset_id=cellset_id,
            cell_properties=cell_properties,
    ...
    def execute_view_values(
        self, cube_name: str, view_name: str, private: bool = False, **kwargs
    ):
    ...
    def execute_mdx_rows_and_values(
        self, mdx: str, element_unique_names: bool = True, **kwargs
    ) -> CaseAndSpaceInsensitiveTuplesDict:
        ...
        return self.extract_cellset_rows_and_values(
            cellset_id, element_unique_names, delete_cellset=True, **kwargs
        )
    ...
    def execute_mdx_csv(self, mdx: str, **kwargs) -> str:
        ...
        return self.extract_cellset_csv(
            cellset_id=cellset_id, delete_cellset=True, **kwargs
        )
    ...
    def execute_view_csv(
        ...
        cellset_id = self.create_cellset_from_view(
            cube_name=cube_name, view_name=view_name, private=private
        )
        return self.extract_cellset_csv(
            cellset_id=cellset_id, delete_cellset=True, **kwargs
        )
    ...
    def execute_view_dataframe_pivot(
        ...
        **kwargs,
        ...
        return self.extract_cellset_dataframe_pivot(
            cellset_id=cellset_id, dropna=dropna, fill_value=fill_value, **kwargs
        )

    def execute_mdx_dataframe_pivot(
        self, mdx: str, dropna: bool = False, fill_value: bool = None
    ) -> pd.DataFrame:
        ...
        return self.extract_cellset_dataframe_pivot(
            cellset_id=cellset_id, dropna=dropna, fill_value=fill_value
        )
    ...
    def execute_mdx_rows_and_values_string_set(
        self, mdx: str, exclude_empty_cells: bool = True, **kwargs
        ...
        rows_and_values = self.execute_mdx_rows_and_values(
            mdx, element_unique_names=False, **kwargs
        )
        return self._extract_string_set_from_rows_and_values(
            rows_and_values, exclude_empty_cells
        )
    ...
    def execute_view_rows_and_values_string_set(
        ...
        **kwargs,
        ...
        rows_and_values = self.execute_view_rows_and_values(
            cube_name, view_name, private, False, **kwargs
        )
        return self._extract_string_set_from_rows_and_values(
            rows_and_values, exclude_empty_cells
        )
    ...
    def extract_cellset_raw(
        ...
        **kwargs,
        ...
        expand_elem_properties = (
            ";$expand=Element($select={elem_properties})".format(
        ...
        if self._rest._sandbox == None:
            url = (
                "/api/v1/Cellsets('{cellset_id}')?$expand="
                ...
                "Cells($select={cell_properties}{top_cells}{skip_cells})".format(
                    cellset_id=cellset_id,
            ...
            url = (
                ...
                "&!sandbox={sandbox_name}".format(
                    ...
                    top_cells=f";$top={top}" if top else "",
                    skip_cells=f";$skip={skip}" if skip else "",
                    sandbox_name=format_url(self._rest._sandbox),
                )
            )
    ...
    def extract_cellset_values(self, cellset_id: str, **kwargs):
        ...
        if self._rest._sandbox == None:
            url = format_url(
                "/api/v1/Cellsets('{}')?$expand=Cells($select=Value)", cellset_id
            )
        else:
            url = format_url(
    ...
    def extract_cellset_rows_and_values(
        self, cellset_id: str, element_unique_names: bool = True, **kwargs
    ) -> CaseAndSpaceInsensitiveTuplesDict:
        ...
        if self._rest._sandbox == None:
            url = (
                ...
                "Cells($select=Value)".format(
                    cellset_id, "UniqueName" if element_unique_names else "Name"
                )
            )
        else:
            url = (
        ...
        element_names_by_row = [
            tuple(
                member["Element"]["UniqueName" if element_unique_names else "Name"]
                for member in tupl["Members"]
            )
            for tupl in rows
        ]
    ...
    def extract_cellset_composition(self, cellset_id: str, **kwargs):
        ...
        if self._rest._sandbox == None:
            ...
            url = (
                ...
                "&!sandbox={}".format(cellset_id, format_url(self._rest._sandbox))
            )
        response = self._rest.GET(url=url, **kwargs)
        response_json = response.json()
        ...
        if response_json["Axes"][0]["Hierarchies"]:
            columns = [
                ...
                for hierarchy in response_json["Axes"][0]["Hierarchies"]
            ]
        ...
        if len(response_json["Axes"]) > 2:
            titles = [
                hierarchy["UniqueName"]
                for hierarchy in response_json["Axes"][2]["Hierarchies"]
            ]
    ...
    def extract_cellset_cellcount(self, cellset_id: str, **kwargs) -> int:
        ...
        if self._rest._sandbox == None:
            ...
            url = "/api/v1/Cellsets('{}')/Cells/$count?!sandbox={}".format(
                cellset_id, format_url(self._rest._sandbox)
            )
    ...
    def extract_cellset_power_bi(self, cellset_id: str, **kwargs) -> pd.DataFrame:
        ...
        column_headers = [
            tupl["Members"][0]["Name"] for tupl in response_json["Axes"][0]["Tuples"]
        ]
        row_headers = [
            hierarchy["Name"] for hierarchy in response_json["Axes"][1]["Hierarchies"]
        ]
        cell_values = [cell["Value"] for cell in response_json["Cells"]]
        ...
        element_names_by_row = [
            tuple(member["Name"] for member in tupl["Members"]) for tupl in rows
        ]
    ...
    def extract_cellset_dataframe_pivot(
        self, cellset_id: str, dropna: bool = False, fill_value: bool = False, **kwargs
    ) -> pd.DataFrame:
        ...
        data = self.extract_cellset(
            cellset_id=cellset_id, delete_cellset=False, **kwargs
        )
        ...
        return pd.pivot_table(
            data=df,
            index=[
                dimension_name_from_element_unique_name(hierarchy_unique_name)
                for hierarchy_unique_name in rows
            ],
            columns=[
                dimension_name_from_element_unique_name(hierarchy_unique_name)
                for hierarchy_unique_name in columns
            ],
            values=["Values"],
            dropna=dropna,
    ...
    def create_cellset(self, mdx: str, **kwargs) -> str:
        ...
        if self._rest._sandbox == None:
            ...
            url = format_url("/api/v1/ExecuteMDX?!sandbox={}", self._rest._sandbox)
        data = {"MDX": mdx}
    ...
    def create_cellset_from_view(
        self, cube_name: str, view_name: str, private: bool, **kwargs
    ) -> str:
        ...
        if self._rest._sandbox == None:
            url = format_url(
                "/api/v1/Cubes('{cube_name}')/{views}('{view_name}')/tm1.Execute",
                cube_name=cube_name,
            ...
            url = format_url(
                ...
                views="PrivateViews" if private else "Views",
                view_name=view_name,
                sandbox_name=self._rest._sandbox,
            )
    ...
    def delete_cellset(self, cellset_id: str,sandbox_name:str=None, **kwargs) -> Response:
        ...
        if self._rest._sandbox == None:
            ...
            url = format_url(
                "/api/v1/Cellsets('{}')?!sandbox={}", cellset_id, self._rest._sandbox
            )
    ...
    def deactivate_transactionlog(self, *args: str, **kwargs) -> Response:
        ...
        return self.write_values(
            cube_name="}CubeProperties", cellset_as_dict=updates, **kwargs
        )
    ...
    def get_view_content(
        ...
        warnings.simplefilter("always", PendingDeprecationWarning)
        warnings.warn(
            "Function deprecated. Use execute_view instead.", PendingDeprecationWarning
        )
        warnings.simplefilter("default", PendingDeprecationWarning)
