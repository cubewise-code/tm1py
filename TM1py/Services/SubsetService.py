...
class SubsetService(ObjectService):
    ...
    def update(self, subset: Subset, private: bool = False, **kwargs) -> Response:
        ...
        if subset.is_static:
            ...
            subset = Subset(subset_name=subset.name, dimension_name=subset.dimension_name, hierarchy_name=subset.hierarchy_name, alias=subset.alias, expression=subset.expression)
        subsets = "PrivateSubsets" if private else "Subsets"
        url = format_url(
            "/Dimensions('{}')/Hierarchies('{}')/{}('{}')",
    ...
    def update_static_elements(
        ...
        elements = [{"@odata.id": format_url("Dimensions('{}')/Hierarchies('{}')/Elements('{}')", dimension_name, hierarchy_name, element)} for element in elements]
