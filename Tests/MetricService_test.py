...
class TestMetricService(unittest.TestCase):
    ...
    def test_by_rule_returns_list_on_both_versions(self):
        ...
        # read/shape path serves both; when populated, records carry these keys.
    ...
    def test_rule_stats_lifecycle_v12(self):
        ...
        try:
            ...
            # }StatsByRule is read the same way as on v11; populating it needs rule
            # activity + the ~60s sampling interval, so just assert the read shape.
            ...
            # only delete }StatsByRule if THIS test created it (never touch a
            # pre-existing one)
