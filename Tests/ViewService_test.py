...
class TestViewService(unittest.TestCase):
    ...
    dimension_names = ["TM1py_Tests_View_Dimension1", "TM1py_Tests_View_Dimension2", "TM1py_Tests_View_Dimension3"]
    ...
    def setUpClass(cls):
        ...
        for i in range(3):
            ...
            hierarchy = Hierarchy(dimension_name=cls.dimension_names[i], name=cls.dimension_names[i], elements=elements)
            dimension = Dimension(name=cls.dimension_names[i], hierarchies=[hierarchy])
    ...
    def setUp(self):
        for private in (True, False):
            ...
            native_view = NativeView(cube_name=self.cube_name, view_name=self.native_view_name)
            ...
            if not self.tm1.dimensions.subsets.exists(
                subset_name=subset.name, dimension_name=subset.dimension_name, private=False
            ):
                self.tm1.dimensions.subsets.create(subset, private=False)
            native_view.add_row(dimension_name=self.dimension_names[0], subset=subset)
            ...
            native_view.add_title(dimension_name=self.dimension_names[1], subset=subset, selection="element123")
            ...
            if not self.tm1.dimensions.subsets.exists(
                ...
                self.tm1.dimensions.subsets.create(subset, private=False)
            native_view.add_column(dimension_name=self.dimension_names[2], subset=subset)
    ...
    def test_view_exists(self):
        for private in (True, False):
            self.assertTrue(
                self.tm1.views.exists(cube_name=self.cube_name, view_name=self.native_view_name, private=private)
            )
            self.assertTrue(
                self.tm1.views.exists(cube_name=self.cube_name, view_name=self.mdx_view_name, private=private)
            )
        exists_as_private, exists_as_public = self.tm1.views.exists(
            cube_name=self.cube_name, view_name=self.mdx_view_name
        )
        self.assertTrue(exists_as_private)
        self.assertTrue(exists_as_public)
        exists_as_private, exists_as_public = self.tm1.views.exists(
            cube_name=self.cube_name, view_name=self.native_view_name
        )
        self.assertTrue(exists_as_private)
        self.assertTrue(exists_as_public)
    ...
    def test_get_native_view(self):
        for private in (True, False):
            # generic get
            view = self.tm1.views.get(cube_name=self.cube_name, view_name=self.native_view_name, private=private)
    ...
    def test_get_mdx_view(self):
        for private in (True, False):
            # generic get
            view = self.tm1.views.get(cube_name=self.cube_name, view_name=self.mdx_view_name, private=private)

            # get mdx view
            mdx_view = self.tm1.views.get_mdx_view(
                cube_name=self.cube_name, view_name=self.mdx_view_name, private=private
            )
    ...
    def test_update_nativeview(self):
        for private in (True, False):
            ...
            # modify it
            native_view_original.remove_row(dimension_name=self.dimension_names[0])
            subset = AnonymousSubset(
                dimension_name=self.dimension_names[0],
                elements=["Element 1", "Element 2", "Element 3", "Element 4", "Element 5"],
            )
            native_view_original.add_column(dimension_name=self.dimension_names[0], subset=subset)
            # update it on Server
            self.tm1.views.update(view=native_view_original, private=private)
    ...
    def test_update_mdxview(self):
        for private in (True, False):
            ...
            data_mdx_original = self.tm1.cells.execute_view(
                cube_name=self.cube_name, view_name=mdx_view_original.name, private=private
            )
            mdx = (
                "SELECT "
                ...
                "FROM [{}] "
                "WHERE ([{}].[Element172])".format(
                    self.dimension_names[0], self.dimension_names[1], self.cube_name, self.dimension_names[2]
                )
            )
            ...
            data_mdx_updated = self.tm1.cells.execute_view(
                cube_name=self.cube_name, view_name=mdx_view_updated.name, private=private
            )
            # Sum up all the values from the views
    ...
    def test_update_mdx_view_with_properties(self):
        mdx_view = self.tm1.views.get_mdx_view(cube_name=self.cube_name, view_name=self.mdx_view_name, private=False)
        properties = {
            "Meta": {
                "ExpandAboves": {
                    f"[{self.dimension_names[0]}].[{self.dimension_names[0]}]": False,
        ...
        self.tm1.views.update(view=mdx_view, private=False)
        retrieved = self.tm1.views.get_mdx_view(cube_name=self.cube_name, view_name=self.mdx_view_name, private=False)
        self.assertIsInstance(retrieved, MDXView)
        self.assertEqual(self.mdx_view_name, retrieved.name)

    def test_search_subset_in_native_views(self):

        private_views, public_views = self.tm1.views.search_subset_in_native_views(
            dimension_name=self.dimension_names[2], subset_name=self.subset_name
        )
    ...
    def test_search_subset_in_native_views_for_cube(self):
        ...
        private_views, public_views = self.tm1.views.search_subset_in_native_views(
            dimension_name=self.dimension_names[2], cube_name=self.cube_name, subset_name=self.subset_name
        )
    ...
    def test_search_subset_in_native_views_no_findings(self):
        ...
        private_views, public_views = self.tm1.views.search_subset_in_native_views(
            dimension_name=self.dimension_names[2], subset_name="NotASubset"
        )
