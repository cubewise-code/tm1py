...
class TestSubsetService(unittest.TestCase):
    ...
    def test_create_and_delete_subset_static_private(self):
        ...
        static_subset = Subset(
            dimension_name=self.dimension_name, subset_name="subset1", elements=["USD", "EUR", "NZD", "Dum'my"]
        )

        response = self.tm1.dimensions.subsets.create(subset=static_subset, private=private)
        self.assertTrue(response.ok)

        response = self.tm1.dimensions.hierarchies.subsets.delete(
            dimension_name=self.dimension_name, subset_name=static_subset.name, private=private
        )
        self.assertTrue(response.ok)
    ...
    def test_create_and_delete_subset_dynamic_private(self):
        ...
        response = self.tm1.dimensions.subsets.create(subset=dynamic_subset, private=private)
        self.assertTrue(response.ok)

        response = self.tm1.dimensions.hierarchies.subsets.delete(
            dimension_name=self.dimension_name, subset_name=dynamic_subset.name, private=private
        )
        self.assertTrue(response.ok)
    ...
    def test_exists_private(self):
        private = True
        self.assertTrue(
            self.tm1.dimensions.subsets.exists(
                subset_name=self.static_subset.name, dimension_name=self.dimension_name, private=private
            )
        )
        self.assertTrue(
            self.tm1.dimensions.subsets.exists(
                subset_name=self.dynamic_subset.name, dimension_name=self.dimension_name, private=private
            )
        )
        self.assertFalse(
            self.tm1.dimensions.subsets.exists(subset_name="wrong", dimension_name=self.dimension_name, private=private)
        )
    ...
    def test_get_subset_private(self):
        ...
        s = self.tm1.dimensions.hierarchies.subsets.get(
            dimension_name=self.dimension_name, subset_name=self.subset_name_static, private=private
        )
        self.assertEqual(self.static_subset, s)
        s = self.tm1.dimensions.hierarchies.subsets.get(
            dimension_name=self.dimension_name, subset_name=self.subset_name_dynamic, private=private
        )
    ...
    def test_update_subset_dynamic_private(self):
        ...
        # Check after update
        self.assertEqual(s.expression, "{{ [{}].[EUR], [{}].[USD] }})".format(self.dimension_name, self.dimension_name))
    ...
    def test_update_subset_dynamic_public(self):
        ...
        subset.expression = "{{ [{}].[EUR], [{}].[USD] }})".format(self.dimension_name, self.dimension_name)
        # Update it
        self.tm1.dimensions.hierarchies.subsets.update(subset=subset, private=private)
        # Get it again
        subset = self.tm1.dimensions.hierarchies.subsets.get(
            dimension_name=self.dimension_name, subset_name=self.subset_name_dynamic, private=private
        )
    ...
    def test_make_static_private(self):
        ...
        subset = self.tm1.dimensions.subsets.get(
            subset_name=self.dynamic_subset.name, dimension_name=self.dynamic_subset.dimension_name, private=private
        )
    ...
    def test_get_all_names_private(self):
        ...
        subset_names = self.tm1.dimensions.subsets.get_all_names(
            dimension_name=self.dimension_name, hierarchy_name=self.dimension_name, private=private
        )
    ...
    def test_delete_elements_from_static_subset_public(self):
        ...
        self.tm1.dimensions.subsets.create(subset=static_subset, private=private)
        subset = self.tm1.dimensions.subsets.get(
    ...
    def test_get_element_names_static(self):
        element_names = self.tm1.subsets.get_element_names(
            self.dimension_name, self.dimension_name, self.subset_name_static, False
        )
    ...
    def test_get_element_names_dynamic(self):
        element_names = self.tm1.subsets.get_element_names(
            self.dimension_name, self.dimension_name, self.subset_name_dynamic, False
        )
    ...
    def test_create_subset_with_url_unfriendly_characters_in_elements_dynamic(self):
        ...
        subset = self.tm1.subsets.get(
            dimension_name=self.unfriendly_dimension_name, subset_name=self.unfriendly_subset_name
        )
        self.assertEqual(self.unfriendly_subset_name, subset.name)
