...
class Process:
    ...
    def create_empty_process(self):
        # Field Parameters
        self.name = ''
        self.has_security_access = False
        self.ui_data = "CubeAction=1511€DataAction=1503€CubeLogChanges=0€"
        self.prolog_procedure = Process.auto_generated_string()
        self.metadata_procedure = Process.auto_generated_string()
        self.data_procedure = Process.auto_generated_string()
        self.epilog_procedure = Process.auto_generated_string()
        self.datasource_type = "ASCII"
        self.datasource_ascii_decimal_separator = ","
        self.datasource_ascii_delimiter_char = ";"
        self.datasource_ascii_delimiter_type ="Character"
        self.datasource_ascii_header_records = 0
        self.datasource_ascii_quote_character="\""
        self.datasource_ascii_thousand_separator="."
        self.datasource_data_source_name_for_client = ''
        self.datasource_data_source_name_for_server = ''

        # List Parameters
        self.parameters = []
        self.variables = []
        self.variables_ui_data = []

    def create_process_from_json(self, body_as_json):
        body_as_dict = json.loads(body_as_json)
        self.create_process_from_dict(body_as_dict)

    def create_process_from_dict(self, body_as_dict):
        ''' construct the class attributes from a given dictionary

        '''
        self.counter_variables = 0
        self.counter_parameters = 0
        self.name = body_as_dict['Name']
        self.has_security_access = body_as_dict['HasSecurityAccess']
        self.ui_data = body_as_dict['UIData']
        self.variables_ui_data = body_as_dict['VariablesUIData']
        self.parameters = []
        for item in body_as_dict['Parameters']:
            self.parameters.append(item)
            self.counter_parameters += 1
        self.variables = []
        for item in body_as_dict['Variables']:
            self.variables.append(item)
            self.counter_variables += 1
        self.prolog_procedure = body_as_dict['PrologProcedure']
        self.metadata_procedure = body_as_dict['MetadataProcedure']
        self.data_procedure = body_as_dict['DataProcedure']
        self.epilog_procedure = body_as_dict['EpilogProcedure']
        self.datasource_type = body_as_dict['DataSource']['Type']

        if self.datasource_type == 'ASCII':
            self.datasource_ascii_decimal_separator = body_as_dict['DataSource']['asciiDecimalSeparator']
            self.datasource_ascii_delimiter_char = body_as_dict['DataSource']['asciiDelimiterChar']
            self.datasource_ascii_delimiter_type = body_as_dict['DataSource']['asciiDelimiterType']
            self.datasource_ascii_header_records = body_as_dict['DataSource']['asciiHeaderRecords']
            self.datasource_ascii_quote_character = body_as_dict['DataSource']['asciiQuoteCharacter']
            self.datasource_ascii_thousand_separator = body_as_dict['DataSource']['asciiThousandSeparator']
            self.datasource_data_source_name_for_client = body_as_dict['DataSource']['dataSourceNameForClient']
            self.datasource_data_source_name_for_server = body_as_dict['DataSource']['dataSourceNameForServer']

        elif self.datasource_type == 'ODBC':
            self.datasource_data_source_name_for_client = body_as_dict['DataSource']['dataSourceNameForClient']
            self.datasource_data_source_name_for_server = body_as_dict['DataSource']['dataSourceNameForServer']
            self.datasource_password = body_as_dict['DataSource']['password']
            self.datasource_user_name = body_as_dict['DataSource']['userName']
            self.datasource_query = body_as_dict['DataSource']['query']
            self.datasource_uses_unicode = body_as_dict['DataSource']['usesUnicode']

        elif self.datasource_type == 'TM1CubeView':
            self.datasource_data_source_name_for_client = body_as_dict['DataSource']['dataSourceNameForServer']
            self.datasource_data_source_name_for_server = body_as_dict['DataSource']['dataSourceNameForServer']
            self.datasource_view = body_as_dict['DataSource']['view']
